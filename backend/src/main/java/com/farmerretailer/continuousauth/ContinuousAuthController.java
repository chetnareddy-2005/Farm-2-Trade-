    ...
    public ResponseEntity<?> evaluateTelemetry(@RequestBody ContinuousAuthRequestDTO requestDTO) {
        try {
            ...
            if ("HIGH".equals(response.getRiskLevel())) {
                isAnomaly = true;
                reason = "ML Service flagged HIGH risk (score: " + response.getScore() + ")";
            } else if ("MEDIUM".equals(response.getRiskLevel())) {
                isAnomaly = true;
                reason = "ML Service flagged MEDIUM risk (score: " + response.getScore() + ")";
            } else if (mouseSpeed != null && mouseSpeed > 2000) {
                isAnomaly = true;
            ...
            if (isAnomaly) {
                ...
                if ("HIGH".equals(response.getRiskLevel())) {
                    ...
                    System.out.println("[Telemetry Debug] Triggering Session Termination (ML Service HIGH Risk)");
                } else if ("MEDIUM".equals(response.getRiskLevel()) || currentCount >= 3) {
                    response.setRiskLevel("MEDIUM");
                    System.out.println("[Telemetry Debug] Triggering OTP (ML Service MEDIUM Risk or Anomaly Count reached 3)");
                } else {
