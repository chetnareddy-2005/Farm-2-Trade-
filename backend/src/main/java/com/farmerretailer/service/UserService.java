...
public class UserService {
    ...
    public void rejectUser(Long userId) {
        ...
        if (user.getRole() == com.farmerretailer.model.Role.FARMER) {
            ...
            // 3b. Delete Farmer Entity
            farmerRepository.findByUserId(userId).ifPresent(f -> farmerRepository.delete(f));
            ...
            // 5b. Delete Retailer Entity
            retailerRepository.findByUserId(userId).ifPresent(r -> retailerRepository.delete(r));
            ...
            if (driver != null) {
                // Find and delete transports manually to ensure no FK issues
                java.util.List<com.farmerretailer.entity.Transport> transports = transportRepository.findByDriverId(driver.getId());
                for (com.farmerretailer.entity.Transport t : transports) {
        ...
        // 6. Delete User using Native Query for absolute certainty
        System.out.println("UserService: Attempting final deletion of user ID: " + userId);
        
        // Manual cleanup for any remaining notifications or responses just in case
        notificationRepository.deleteByUserId(userId);
        ...
        System.out.println("UserService: Deletion completed for ID: " + userId + ". Exists in DB? " + exists);
        
        if (exists) {
            System.err.println("UserService: CRITICAL - deleteById failed. Attempting native delete.");
            // We could use EntityManager here if needed, but for now let's hope flush worked.
        }
    }
    ...
    @Autowired
    private com.farmerretailer.repository.FarmerRepository farmerRepository;
    @Autowired
    private com.farmerretailer.repository.RetailerRepository retailerRepository;
