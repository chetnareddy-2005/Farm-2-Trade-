...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(o) FROM Order o WHERE o.retailer.id = :retailerId")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(o) FROM Order o WHERE o.retailer.id = :retailerId AND (LOWER(o.status) = 'pending' OR LOWER(o.status) = 'confirmed')")
    long countPendingByRetailerId(@Param("retailerId") Long retailerId);

    @org.springframework.data.jpa.repository.Query("SELECT COUNT(o) FROM Order o WHERE o.retailer.id = :retailerId AND (LOWER(o.status) = 'shipped' OR LOWER(o.status) = 'in_transit')")
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT AVG(TIMESTAMPDIFF(SECOND, order_date, delivered_at) / 86400) FROM orders WHERE retailer_id = :retailerId AND delivered_at IS NOT NULL", nativeQuery = true)
