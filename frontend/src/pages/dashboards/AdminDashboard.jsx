...
const AdminDashboard = () => {
    ...
    const handleViewDocument = async (userId, fileName) => {
        setDocName(fileName);
        setIsDocLoading(true);
        setShowDocModal(true);
        
        try {
            const res = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/api/users/${userId}/document`);
            if (res.ok) {
                ...
                alert("Could not load document. It may have been lost or is inaccessible.");
    ...
    const handleDeleteUser = async (user) => {
        ...
        try {
            ...
            if (response.ok) {
                console.log(`[Admin] Successfully deleted user ID: ${user.id} (type: ${typeof user.id})`);
                alert("User deleted successfully.");
                
                // Refresh local state immediately with robust ID comparison
                setUserList(prev => {
                    const userIdStr = String(user.id);
                    const updated = prev.filter(u => String(u.id) !== userIdStr);
                    console.log(`[Admin] Local state updated. Before: ${prev.length}, After: ${updated.length}`);
                    return updated;
                });
                // Wait 500ms before re-fetching from server to ensure DB consistency
                setTimeout(() => {
                    if (listModalTitle === "All Registered Users") {
                        fetchAllUsers();
                    } else {
            ...
            <aside style={{
                ...
                <div style={{ padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <img src={`${import.meta.env.BASE_URL}assets/farm_trade_leaf.png`} alt="Logo" style={{ width: '32px', height: '32px' }} />
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            {isDocLoading ? (
                                <div style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>Loading document securely...</div>
                            ) : !docUrl ? (
                                <div style={{ color: '#ef4444', fontWeight: '600' }}>Failed to load preview.</div>
