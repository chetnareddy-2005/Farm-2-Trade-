...
import { LayoutDashboard, ShoppingBag, User, LogOut, Package, HelpCircle, ChevronRight, Plus, Bell, Truck, TrendingUp } from 'lucide-react';
...
const RetailerDashboard = () => {
    ...
    const fetchUserProfile = async () => {
        try {
            ...
            if (res.ok) {
                ...
                console.log("[Retailer Profile Debug] Fetched profile:", data);
                setUser(data);
    ...
    const fetchWithAuth = async (url, options = {}) => {
        ...
        try {
            ...
            if (res.status === 401) {
                console.warn(`[Auth Fallback] 401 Unauthorized detected for ${url}.`);
                handleUnauthorized();
            }
            return res;
        } catch (err) {
    ...
    const [stats, setStats] = useState({ totalOrders: 0, pendingOrders: 0, activeOrders: 0, deliveredOrders: 0 });
    ...
    useEffect(() => {
        ...
        if (!storedUser || !token) {
            console.warn("[Auth Guard] Missing session. Redirecting to login.");
            navigate('/login');
            return;
        }
        ...
        try {
            ...
            console.error("[Auth Guard] Parse error", e);
            handleUnauthorized();
            return;
        }
                                    ...
                                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                                        {filters.selectedFarmer && (
                                            <button
                                                ...
                                                style={{ padding: '0.5rem 1rem', borderRadius: '12px', border: '1px solid #D1D5DB', backgroundColor: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', fontWeight: '500' }}>
                                        ...
                                        >
                                            <TrendingUp size={18} /> Refresh Market
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'grid', gap: '1.5rem' }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ fontSize: '1rem', color: 'var(--text-primary)', padding: '0.75rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            ...
                                            {user?.documentName && (
                                                <button
                                                    onClick={() => handleViewDocument(user.id, user.documentName)}
                                                    ...
                                                    style={{ 
                                                        ...
                                                        background: 'none', 
                                                        border: 'none', 
                                                        cursor: isDocLoading ? 'wait' : 'pointer', 
                                                        fontSize: '0.875rem' 
                                                    }}
                                                >
