...
const FarmerDashboard = () => {
    ...
    const fetchWithAuth = async (url, options = {}) => {
        ...
        try {
            ...
            if (res.status === 401) {
                console.warn(`[Auth Fallback] 401 Unauthorized detected for ${url}. Redirecting to landing page.`);
                handleUnauthorized();
            }
            return res;
        } catch (err) {
    ...
    const handleViewDocument = async (userId, fileName) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setDocUrl(blobUrl);
                // Open in new tab or show in modal (here we just open in new tab for simplicity if it's the profile)
    ...
    useEffect(() => {
        ...
        if (!storedUser || !token) {
            console.warn("[Auth Guard] Missing user or token in localStorage. Redirecting to login.");
            navigate('/login');
            return;
        }
        ...
        try {
            ...
            console.error("[Auth Guard] Failed to parse user data", e);
            handleUnauthorized();
            return;
        }
